...
게임 export를 반영하면 마을 카드에서 본 마을 연구소, 펫, 장인기지 장인과 장인기지 연구소의 작업 가능 여부도 확인할 수 있습니다. 시설이나 전용 슬롯이 해금된 경우에만 표시하며, 활성 타이머가 없을 때 `업그레이드 가능`으로 표시합니다. 펫과 연구는 본 마을 장인을 사용하지 않으므로 대기 장인 계산에서 제외하고, 장인기지 장인 수와 작업 상태는 본 마을과 별도로 계산합니다. 기존에 저장된 export에는 이 상태가 없을 수 있으므로 기능 업데이트 후 새 게임 export를 한 번 반영해야 합니다.
