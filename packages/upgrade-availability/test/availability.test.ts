...
test("[DISPLAY-SLOT-001] observes Goblin helpers from concurrent work", () => {
  const observations = observeAvailability([
    {
      builders: { free: 0, total: 7, regularTotal: 6 },
      upgradeSlots: { laboratory: { available: false, active: 2, total: 2 } },
    },
  ]);
...
test("[DISPLAY-SLOT-002] offers a second research slot when one account proves the event is active", () => {
  const result = applyDisplayOptions(
    {
      builders: { free: 0, total: 6, regularTotal: 6 },
      upgradeSlots: { laboratory: { available: false, active: 1, total: 1 } },
    },
...
test("[DISPLAY-SLOT-003] offers a Goblin Builder only after all eligible regular builders are busy", () => {
  ...
  const busy = applyDisplayOptions(
    { builders: { free: 0, total: 6, regularTotal: 6 } },
    observations,
    defaultDisplayOptions,
  );
  const partiallyFree = applyDisplayOptions(
    { builders: { free: 1, total: 6, regularTotal: 6 } },
    observations,
    defaultDisplayOptions,
  );
  const ineligible = applyDisplayOptions(
    { builders: { free: 0, total: 4, regularTotal: 4 } },
    observations,
    defaultDisplayOptions,
  );
...
test("[DISPLAY-SLOT-004] respects disabled inference options", () => {
  ...
  const result = applyDisplayOptions(
    account,
    { goblinResearcher: true, goblinBuilder: true },
    { goblinResearcher: false, goblinBuilder: false },
  );
...
test("[DISPLAY-SUMMARY-001] totals idle Home Village and Builder Base slots separately", () => {
  ...
  assert.deepEqual(
    summarizeAvailability(accounts, { goblinResearcher: false, goblinBuilder: false }, defaultDisplayOptions),
    {
      homeVillage: 5,
      builderBase: 2,
    },
...
test("[DISPLAY-FILTER-001] distinguishes Home Village availability from any available slot", () => {
  const account = {
    ...
    upgradeSlots: {
      ...
      petHouse: null,
      builderBase: { builders: { free: 1, total: 2 }, laboratory: null },
    },
  };
