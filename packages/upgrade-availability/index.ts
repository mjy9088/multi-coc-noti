...
export type UpgradeTimelinePoint = {
  at: number;
  activeHome: number;
  activeAll: number;
...
export function observeAvailability(accounts: AvailabilityAccount[]): AvailabilityObservations {
  return {
    ...
    goblinBuilder: accounts.some(
      (account) => account.builders.total > (account.builders.regularTotal ?? account.builders.total),
    ),
  };
}
...
export function applyDisplayOptions(
  ...
  const inferGoblinBuilder =
    options.goblinBuilder && observations.goblinBuilder && regularBuilders >= 5 && activeBuilders >= regularBuilders;
  const builderTotal = inferGoblinBuilder
    ? Math.max(account.builders.total, regularBuilders + 1)
    : account.builders.total;
  ...
  const inferGoblinResearcher =
    options.goblinResearcher && observations.goblinResearcher && (laboratory?.active || 0) === 1;

  return {
    builders,
    laboratory:
      laboratory && inferGoblinResearcher
...
export function buildUpgradeChartData(
  upgrades: UpgradeChartInput[],
  idleHome: number,
  idleBuilder: number,
  now: number,
  binCount = 8,
): { timeline: UpgradeTimelinePoint[]; bins: CompletionBin[] } {
  const isHome = (upgrade: UpgradeChartInput) => upgrade.base !== "builder";
  const active = upgrades.filter(
    (upgrade) => Number.isFinite(new Date(upgrade.finishAt).getTime()) && new Date(upgrade.finishAt).getTime() > now,
  );
  ...
  const timeline = [now, ...finishTimes].map((at) => {
    ...
    return {
      at,
      activeHome: activeAt.filter(isHome).length,
      activeAll: activeAt.length,
      availableHome: idleHome + completedHome,
      availableAll: idleHome + idleBuilder + completed.length,
    };
  });
  ...
  const bins = Array.from({ length: count }, (_, index) => {
    ...
    const entries = active.filter((upgrade) => {
