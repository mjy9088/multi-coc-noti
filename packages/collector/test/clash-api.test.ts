...
test("[API-PROFILE-001] fetches and maps an official player profile with an encoded tag", async () => {
  ...
  const profile = await fetchPlayerProfile(account, {
    ...
    fetchImpl: async (url, options) => {
      ...
      return new Response(JSON.stringify({ name: "Real Village", tag: "#2ABC", townHallLevel: 17, expLevel: 241, trophies: 5100, bestTrophies: 5300, league: { name: "Legend League" }, warStars: 1200, donations: 500, donationsReceived: 300, clanCapitalContributions: 42000 }));
    },
  });
  assert.ok(request);
  ...
  assert.deepEqual(profile, { name: "Real Village", tag: "#2ABC", townHall: 17, level: 241, stats: { trophies: 5100, bestTrophies: 5300, league: "Legend League", warStars: 1200, donations: 500, donationsReceived: 300, capitalContributions: 42000 } });
});
