...
type LegacyBundle = {
  ...
  villageExports: Array<{
    playerTag: string;
    exportedAt: string;
    raw: unknown;
    normalized?: VillageHistoryBundle["villageExports"][number]["normalized"];
  }>;
...
function canonicalizeSettings(bundle: VillageHistoryBundle): VillageHistoryBundle {
  ...
  for (const item of bundle.villageExports) {
    ...
    for (const oldUpgrade of item.normalized.upgrades || []) {
      const match = reparsed.upgrades.find(
        (upgrade) =>
          upgrade.name === oldUpgrade.name &&
          upgrade.type === oldUpgrade.type &&
          upgrade.base === oldUpgrade.base &&
          upgrade.nextLevel === oldUpgrade.nextLevel &&
          upgrade.finishAt === oldUpgrade.finishAt,
      );
  ...
  return {
    ...bundle,
    upgradeSettings: bundle.upgradeSettings.map((setting) => ({
      ...setting,
      sourceKey: keyMap.get(setting.sourceKey) || setting.sourceKey,
    })),
  };
}
...
async function readBundle(source: string): Promise<VillageHistoryBundle> {
  ...
  if (firstRecord.format === "multi-coc-village-history") {
    ...
    if (legacy.version !== 1 || !legacy.account?.label || !Array.isArray(legacy.villageExports))
      throw new Error(`invalid v1 village history: ${source}`);
    return canonicalizeSettings({
      format: "multi-coc-village-exports",
      version: 2,
      exportedAt: legacy.exportedAt,
      account: legacy.account,
      villageExports: legacy.villageExports.map((item) => ({
        ...item,
        normalized: item.normalized || parseRaw(item.raw, item.exportedAt),
      })),
  ...
  const records = trimmed.split(/\r?\n/).map((line, index) => {
    try {
      ...
      throw new Error(`invalid JSON on line ${index + 1}: ${source}`);
    }
  });
  const header = records.shift();
  if (
    header?.record !== "village" ||
    header.format !== "multi-coc-village-exports" ||
    header.version !== 2 ||
  ...
  const villageExports = records.map((record, index) => {
    if (record.record !== "export" || typeof record.exportedAt !== "string" || !("raw" in record))
      throw new Error(`invalid export record on line ${index + 2}: ${source}`);
    const normalized = parseRaw(record.raw, record.exportedAt);
    if (account.playerTag && normalized.tag !== account.playerTag)
      throw new Error(`player tag mismatch on line ${index + 2}: ${source}`);
  ...
  for (let index = 1; index < villageExports.length; index += 1) {
    if (villageExports[index - 1].exportedAt >= villageExports[index].exportedAt)
      throw new Error(`exports are not strictly chronological: ${source}`);
  }
  return {
    format: "multi-coc-village-exports",
    version: 2,
    ...
    account,
    villageExports,
    upgradeSettings: Array.isArray(header.upgradeSettings)
      ? (header.upgradeSettings as VillageHistoryBundle["upgradeSettings"])
      : [],
  };
}
...
function serialize(bundle: VillageHistoryBundle): string {
  const header = {
    ...
    upgradeSettings: bundle.upgradeSettings,
  };
  return `${[
    header,
    ...bundle.villageExports.map((item) => ({ record: "export", exportedAt: item.exportedAt, raw: item.raw })),
  ]
...
try {
  if (command === "export") {
    ...
    for (const bundle of bundles) {
      ...
      console.log(
        `[history] exported ${bundle.account.label} (${bundle.account.playerTag || bundle.account.id}) -> ${destination}`,
      );
    }
    if (!selector) {
      for (const name of (await readdir(target)).filter(
        (item) => (item.endsWith(".jsonl") || item.endsWith(".json")) && !expectedFiles.has(item),
      )) {
        const candidate = path.join(target, name);
        try {
          const first = (await readFile(candidate, "utf8")).trimStart();
          if (first.includes('"multi-coc-village-exports"') || first.includes('"multi-coc-village-history"'))
