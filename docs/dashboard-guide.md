...
| `Upgrades & alerts` | 활성 업그레이드의 완료 시각과 Bark 알림 시점 설정 |
...
기본 알림은 완료 60분 전, 1분 전과 완료 시점입니다. `Upgrades & alerts`에서 쉼표로 구분한 분 단위 값을 저장합니다.

- `60, 10, 0`: 60분 전, 10분 전, 완료 시점
- `0`: 완료 시점만
- 빈 값: 해당 업그레이드의 미래 알림 끄기

이미 발송된 알림은 변경하지 않으며, 실패한 Bark 요청은 DB에 오류와 다음 재시도 시각을 기록합니다.

## 저장 위치

| 설정 | 저장 위치 |
| --- | --- |
| 계정, 태그, 그룹 순서, 알림 시점 | PostgreSQL |
