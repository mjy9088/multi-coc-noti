...
| `IMPORT-COOLDOWN-001` | Convert exported Clock Tower and helper cooldowns into absolute availability times without inventing unsupported cooldowns. | [Data flow: Server validation](village-data-flow.md#server-validation) | Collector unit |
| `IMPORT-DETAIL-001` | Preserve helper and Hero Equipment identities and levels for useful village detail views. | [Data flow: Server validation](village-data-flow.md#server-validation) | Collector unit |
| `IMPORT-KEY-001` | Keep export upgrade identities stable when unrelated array entries reorder, preserving history and notification settings. | [Data flow: Server validation](village-data-flow.md#server-validation) | Collector unit |
| `IMPORT-DIFF-001` | Summarize upgrade and slot changes against the newest stored export, making Review explain what changed before import. | [Data flow: Standard flow](village-data-flow.md#standard-flow) | Collector unit |
