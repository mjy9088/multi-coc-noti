...
export default function UpgradeAvailabilityPanel({ builders, upgradeSlots }: { builders: BuilderAvailability; upgradeSlots?: UpgradeSlots }) {
  ...
  return <div className="upgrade-availability"><div className="availability-heading"><div className="hammer">◆</div><strong>{t("title")}</strong></div><div className="upgrade-slots">
    ...
    {upgradeSlots?.laboratory && <div className={upgradeSlots.laboratory.available ? "ready" : ""}><span>{t("research")}</span><strong>{laboratoryStatus(upgradeSlots.laboratory)}</strong></div>}
    ...
    {upgradeSlots?.builderBase && <div className={upgradeSlots.builderBase.builders.free > 0 ? "ready" : ""}><span>{t("builderBaseBuilder")}</span><strong>{builderStatus(upgradeSlots.builderBase.builders.free, upgradeSlots.builderBase.builders.total)}</strong></div>}
    {upgradeSlots?.builderBase?.laboratory && <div className={upgradeSlots.builderBase.laboratory.available ? "ready" : ""}><span>{t("builderBaseLab")}</span><strong>{laboratoryStatus(upgradeSlots.builderBase.laboratory)}</strong></div>}
  </div></div>;
}
